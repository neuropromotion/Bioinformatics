...
def shared_mirs(path_tcga_counts=PATH_TCGA_COUNTS, path_gtex_counts=PATH_GTEX_COUNTS, filter = True):
    tcga_loader = Load_TCGA_Data(PATH_TCGA_COUNTS, PATH_TCGA_META)
    gtex_loader = Load_GTEx_Data(PATH_GTEX_COUNTS, PATH_GTEX_META)
    tcga_cpm = tcga_loader.agregate(tcga_loader.CPM(use_shared_mirs=False, log=False))
    gtex_cpm = gtex_loader.agregate(gtex_loader.CPM(use_shared_mirs=False, log=False))

    if filter:
        max_exp_tcga = tcga_cpm.max()
        tcga_filtered = tcga_cpm.loc[:, max_exp_tcga > min_CPM]

        max_exp_gtex = gtex_cpm.max()
        gtex_filtered = gtex_cpm.loc[:, max_exp_gtex > min_CPM]

        tcga_filtered.columns = tcga_filtered.columns.map(standardize_name)
        gtex_filtered.columns = gtex_filtered.columns.map(standardize_name)

        shared_mirs = set(tcga_filtered.columns) & set(gtex_filtered.columns)
        return sorted(list(shared_mirs))

    tcga_cpm.columns = tcga_cpm.columns.map(standardize_name)
    gtex_cpm.columns = gtex_cpm.columns.map(standardize_name)
    return sorted(list(gtex_cpm.columns))
...
class Load_GTEx_Data:
    ...
    def load_counts(self, meta_elig=None, use_shared_mirs=True, filter=True):
        ...
        # paralogous loci map to the same URS -> identical duplicate rows; keep one
        g = g.drop_duplicates(subset="mature_name")
        if use_shared_mirs:
    ...
    def CPM(self, wide=None, smts=None, log=False, use_shared_mirs=True, filter=True):
        if wide is None:
            wide, smts = self.load_counts(use_shared_mirs=use_shared_mirs, filter=filter)
        if smts is None:
...
class Load_TCGA_Data:
    ...
    def load_counts(self, use_shared_mirs=True, filter=True):
    ...
    def CPM(self, df_in=None, log=False, use_shared_mirs=True, filter=True):
        
        if df_in is None:
            wide = self.load_counts(use_shared_mirs=use_shared_mirs, filter=filter)
        else:
...
def compare_gtex_tcga_profiles( 
    ...
    for mir in mir_cols:
        ...
        med_gtex = float(np.mean(x)) if x.size else np.nan
        max_gtex = float(np.max(x)) if x.size else np.nan
        med_tcga = float(np.mean(y)) if y.size else np.nan
        max_tcga = float(np.max(y)) if y.size else np.nan

        x_log = np.log2(x + 1.0)
        y_log = np.log2(y + 1.0)
        med_log_gtex = float(np.mean(x_log)) if x_log.size else np.nan
        med_log_tcga = float(np.mean(y_log)) if y_log.size else np.nan
        ...
        rows.append(
            {
                "mature_name": mir,
                "mean_cpm_gtex": med_gtex,
                "max_cpm_gtex": max_gtex,
                "mean_cpm_tcga": med_tcga,
                "max_cpm_tcga": max_tcga,
                "log2FC_tcga_gtex": log2_fc,
                "mw_statistic": stat,
                "pvalue": pval,
            }
        )
...
def classification_statistics(df_in, threshold=0):
